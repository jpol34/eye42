...
# ---------------------------------------------------------------------------
# splash/plunge inference from the first trick's actual leader / trump call
# ---------------------------------------------------------------------------
...
def test_second_stray_trump_call_before_any_play_does_not_infer_splash():
    """The bidder correctly calls trump first (ruling out splash/plunge,
    since the bidder never calls trump under a real one); a second, stray
    call from the partner arriving before any tile is played must not
    retroactively reclassify the contract or overwrite the real trump."""
    hand = HandState(dealer=3)
    hand.bid(0, marks=2)
    hand.bid_pass(1)
    hand.bid_pass(2)
    hand.bid_pass(3)
    hand.call_trump(0, trump=6)  # correct: the bidder calls trump

    hand.call_trump(2, trump=3)  # stray/misattributed second call from the partner

    assert hand.contract.kind == BidKind.MARKS  # unchanged
    assert hand.contract.trump_caller == 0
    assert hand.trump_tracker.confirmed == 6  # NOT overwritten
    assert IrregularityKind.SPLASH_PLUNGE_INFERRED not in _kinds(hand)
    assert IrregularityKind.TRUMP_CALLED_BY_WRONG_SEAT in _kinds(hand)


def test_partner_leading_first_after_bidder_already_confirmed_trump_stays_out_of_turn():
    """The bidder correctly calls trump (ruling out splash/plunge outright),
    but the bidder's own lead is missed/occluded and the first tile
    perception actually captures comes from the partner -- a genuine
    out-of-turn/attribution error, not evidence of splash/plunge."""
    hand = HandState(dealer=3)
    hand.bid(0, marks=2)
    hand.bid_pass(1)
    hand.bid_pass(2)
    hand.bid_pass(3)
    hand.call_trump(0, trump=6)  # bidder calls trump normally

    hand.play_tile(TilePlayed(player=2, tile=Tile.of(6, 6)))  # partner's lead reaches perception first
    hand.play_tile(TilePlayed(player=3, tile=Tile.of(1, 0)))
    hand.play_tile(TilePlayed(player=0, tile=Tile.of(2, 0)))
    hand.play_tile(TilePlayed(player=1, tile=Tile.of(3, 0)))

    assert hand.contract.kind == BidKind.MARKS  # unchanged, not reclassified as splash
    assert IrregularityKind.OUT_OF_TURN in _kinds(hand)
    assert IrregularityKind.SPLASH_PLUNGE_INFERRED not in _kinds(hand)
